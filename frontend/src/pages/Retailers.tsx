...
const Retailers = () => {
  ...
  const [ratingRetailer, setRatingRetailer] = useState<Retailer | null>(null);
  const [ratingValue, setRatingValue] = useState(0);
  const [showFilters, setShowFilters] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const debouncedSearch = useDebounce(searchQuery);

  const { businessType, status, setSearch } = useRetailerFilters();
  ...
  const handleRatingOpen = (retailer: Retailer) => {
    setRatingRetailer(retailer);
    setRatingValue(Number(retailer.metrics?.averageRating) || 0);
  };

  const handleRatingSubmit = async () => {
    if (!ratingRetailer) return;
    try {
      await updateRating(ratingRetailer.id, ratingValue);
      setRatingRetailer(null);
    } catch (err) {
      // handled by mutation
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={!!ratingRetailer} onOpenChange={(open) => !open && setRatingRetailer(null)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Set Rating</DialogTitle>
            <DialogDescription>
              {ratingRetailer?.businessName}'s performance rating, shown on their profile and in reports.
            </DialogDescription>
          </DialogHeader>
          ...
          <Button onClick={handleRatingSubmit} disabled={isUpdatingRating} className="w-full">
            {isUpdatingRating ? "Saving..." : "Save Rating"}
          </Button>
        </DialogContent>
      </Dialog>
      ...
      <Card>
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex-1 bg-secondary rounded-full h-2">
                          <div
                            ...
                            style={{
                              width: `${((retailer.metrics?.averageRating ?? 0) / 5) * 100}%`,
                            }}
                          />
                        </div>
                        <span className="text-sm font-medium">
                          {retailer.metrics?.averageRating ?? 0}
                        </span>
                      </div>
                    </TableCell>
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem
                            onClick={() => handleRatingOpen(retailer)}
                          >
        ...
        <CardFooter className="flex items-center justify-between px-6 pt-4">
          ...
          <Pagination>
            <PaginationContent className="flex items-center gap-6 flex-1">
              <PaginationItem>
                <Button
                  variant="ghost"
                  size="icon"
