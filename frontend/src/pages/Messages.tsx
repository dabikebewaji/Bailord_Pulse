...
const Messages = () => {
  ...
  useEffect(() => {
    ...
    if (conversations.length > 0) {
      if (userId) {
        ...
        if (targetConversation) {
          ...
          createConversation(userId)
            ...
            .catch((error) => {
              ...
              if (conversations.length > 0) {
                console.log(
                  "Falling back to first conversation:",
                  conversations[0],
                );
        ...
        console.log(
          "No userId, selecting first conversation:",
          conversations[0],
        );
        setSelectedConversation(conversations[0]);
      }
  ...
  useEffect(() => {
    ...
    socket.on("user_typing", (data) => {
      if (
        ...
        data.user_id === Number(selectedConversation.id)
      ) {
        setIsTyping(true);
      }
    });
  ...
  useEffect(() => {
    if (selectedConversation && selectedConversation.id) {
      console.log(
        "Selected conversation changed, fetching messages:",
        selectedConversation,
      );
  ...
  // Opening a conversation should clear its unread badge — markAsRead
  // existed in the hook but nothing ever called it, so badges never
  // cleared no matter how long a conversation had been open and read.
  useEffect(() => {
    if (!selectedConversation || !user?.id) return;
    const unread = messages.filter(
      (m) => !m.isRead && m.recipientId === Number(user.id),
    );
    if (unread.length === 0) return;
    Promise.all(unread.map((m) => markAsRead(m.id))).then(() => {
      fetchConversations();
    });
  }, [
    messages,
      ...
      <div className="grid grid-cols-12 gap-6 h-[calc(100vh-16rem)]">
        ...
        <Card className="col-span-4 p-0 overflow-hidden">
          ...
          <ScrollArea className="h-[calc(100%-5rem)]">
            <div className="p-4 w-4/5 sm:w-full mb-8">
              {conversations
                .filter((conv) =>
                  searchQuery
                    ? (conv.name || "")
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  size="icon"
                  variant={
                    !selectedConversation || !messageInput.trim()
                      ? "outline"
                      : "default"
                  }
                ...
                >
                  <Send
                    className={`h-4 w-4 ${!selectedConversation || !messageInput.trim() ? "text-muted-foreground" : ""}`}
                  />
                </Button>
              </div>
            </div>
