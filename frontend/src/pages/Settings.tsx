...
const Settings = () => {
  ...
  // Notification settings — previously local-only state with no backend at
  // all (toggles visually flipped and reset on reload). Now initialized
  // from the profile and saved immediately on each toggle.
  ...
  // App preferences — same fix as notifications above.
  ...
  useEffect(() => {
    if (user?.notifications) {
      setNotificationSettings({
        ...
        messageAlerts: user.notifications.messages,
        weeklyReports: user.notifications.weeklyReports,
      });
    }
    if (user?.preferences) {
      setAppPreferences({
        compactView: user.preferences.compactView,
  ...
  const handleNotificationChange = async (
    ...
    const next = { ...previous, [setting]: !previous[setting] };
  ...
  const handlePreferenceChange = async (setting: keyof typeof appPreferences) => {
