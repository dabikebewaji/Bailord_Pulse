...
const Analytics = () => {
  ...
  const mapDatasets = (ds: any[] | undefined) =>
    (ds || []).map((d: any, i: number) => ({
      ...
      // Straight segments — smoothed splines overshoot past the actual
      // value around an isolated spike, which reads as inaccurate data.
      tension: 0,
      fill: false,
    }));
  ...
  return (
      <div className="space-y-6">
        ...
        <MetricCard
          ...
          icon={Package}
          trend={{ value: Math.round(metrics?.totalRetailers?.trend || 0) }}
        />
        ...
        <MetricCard
          ...
          icon={TrendingUp}
          trend={{ value: Math.round(metrics?.activeProjects?.trend || 0) }}
        />
        ...
        <MetricCard
          ...
          icon={TrendingUp}
          trend={{ value: Math.round(metrics?.performanceScore?.trend || 0) }}
        />
        ...
        <MetricCard
          ...
          icon={DollarSign}
          trend={{ value: Math.round(metrics?.activeUsers?.trend || 0) }}
        />
      </div>
