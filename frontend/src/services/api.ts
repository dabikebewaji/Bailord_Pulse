...
export const retailerAPI = {
  getAll: (page = 1, limit = 10, filters: { search?: string; status?: 'active' | 'inactive'; businessType?: string } = {}) => {
  ...
  updateMetrics: (id: string, metrics: { averageRating: number }) =>
    api.patch<RetailerResponse>(`/retailers/${id}/metrics`, metrics),

  delete: (id: number) =>
    api.delete<{ status: string, data: null }>(`/retailers/${id}`),

  // The business record linked to the logged-in user (retailer role).
  getMine: () =>
    api.get<RetailerResponse>('/retailers/me'),

  updateMine: (data: Partial<RetailerFormData>) =>
    api.patch<RetailerResponse>('/retailers/me', data),
};
...
export const projectAPI = {
  ...
  // Projects assigned to the logged-in user's linked retailer business.
  getMine: () =>
    api.get<any[]>('/projects/mine'),
};

// Product API — shared catalog. GET is open to any authenticated role
// (retailers browse it to place orders); writes are admin/staff/superadmin.
...
export const productAPI = {
  getAll: (filters: { category?: string; activeOnly?: boolean } = {}) => {
    const params = new URLSearchParams({
      ...(filters.category && { category: filters.category }),
      ...(filters.activeOnly && { activeOnly: 'true' }),
    });
    return api.get<{ status: string; results: number; data: { products: Product[] } }>(`/products?${params}`);
  },
  ...
  create: (data: ProductFormData) =>
    api.post<{ status: string; data: { product: Product } }>('/products', data),

  update: (id: string, data: Partial<ProductFormData>) =>
    api.patch<{ status: string; data: { product: Product } }>(`/products/${id}`, data),

  delete: (id: string) =>
    api.delete<{ status: string; data: null }>(`/products/${id}`),
};
...
export const orderAPI = {
  placeOrder: (items: { productId: string; quantity: number }[]) =>
    api.post<{ status: string; data: { order: Order } }>('/orders', { items }),

  getMine: () =>
    api.get<{ status: string; results: number; data: { orders: Order[] } }>('/orders/mine'),

  getAll: (status?: Order['status']) =>
    api.get<{ status: string; results: number; data: { orders: Order[] } }>('/orders', {
      params: status ? { status } : {},
    }),

  getById: (id: string) =>
    api.get<{ status: string; data: { order: Order } }>(`/orders/${id}`),

  updateStatus: (id: string, status: Order['status']) =>
    api.patch<{ status: string; data: { order: Order } }>(`/orders/${id}/status`, { status }),
};
