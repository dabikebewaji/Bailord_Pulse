...
export function GlobalSearch() {
  ...
  const fallbackCache = (globalThis as any).__globalSearchFallbackCache as {
    ts: number;
    results: SearchResult[];
  };

  const performSearch = useCallback(
    async (searchQuery: string, allowEmpty = false) => {
        ...
        const transformedResults = data.results.map((result: any) => {
          ...
          switch (result.type) {
            ...
            case "project":
              return {
                ...base,
                subtitle: result.dueDate
                  ? `Due ${new Date(result.dueDate).toLocaleDateString()}`
                  : result.subtitle || "",
                timestamp: result.dueDate
                  ? getRelativeTime(result.dueDate)
                  : undefined,
                badge: result.status,
                badgeColor:
                  result.status?.toLowerCase() === "completed"
                    ...
                    : result.status?.toLowerCase() === "in-progress"
                      ? "blue"
                      : result.status?.toLowerCase() === "overdue"
                        ? "red"
                        : "gray",
              };
            case "message":
            case "conversation":
              return {
                ...base,
                subtitle:
                  result.subtitle || result.excerpt || result.lastMessage,
                timestamp: result.timestamp
                  ? getRelativeTime(result.timestamp)
                  : undefined,
              };
            case "user":
              return {
                ...
                subtitle: [
                  ...
                  result.department,
                  result.lastActive
                    ? `Last active ${getRelativeTime(result.lastActive)}`
                    : null,
                ]
                  .filter(Boolean)
                  .join(" • "),
                ...
                badgeColor:
                  result.role?.toLowerCase() === "admin"
                    ? "purple"
                    : result.role?.toLowerCase() === "manager"
                      ? "blue"
                      : "gray",
              };
            default:
  ...
  const getResultUrl = (result: any): string => {
    switch (result.type) {
      ...
      case "project":
        // No per-project detail route exists — project details/editing happen
        // via a modal on the list page, not a dedicated URL.
        return "/projects";
      case "message":
      case "conversation":
  ...
  useEffect(() => {
    if (isOpen && isAuthenticated && (!query || query.trim().length === 0)) {
      const fetchFallbackResults = async () => {
        ...
        if (
          Date.now() - fallbackCache.ts < FALLBACK_TTL &&
          fallbackCache.results.length > 0
        ) {
        ...
        try {
          ...
          const retailers = Array.isArray(retRes.data?.data)
            ? retRes.data.data
            : Array.isArray(retRes.data)
              ? retRes.data
              : [];
          const projects = Array.isArray(projRes.data?.data)
            ? projRes.data.data
          ...
          retailers.slice?.(0, 6)?.forEach((r) => {
            transformed.push({
              ...
              url: `/retailers/${r.id}`,
              subtitle: [r.businessType, r.location]
                .filter(Boolean)
                .join(" • "),
              timestamp: r.lastInteraction || r.createdAt,
              badge: r.status,
              badgeColor:
                r.status?.toLowerCase() === "active" ? "green" : "gray",
            } as SearchResult);
          });
          ...
          projects.slice(0, 6).forEach((p) => {
            transformed.push({
              ...
              badge: p.status,
              badgeColor:
                p.status?.toLowerCase() === "completed" ? "green" : "blue",
            } as SearchResult);
          });
      ...
      <div className="relative">
        ...
        <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
          {query && (
            ...
            >
              <X className="h-4 w-8 p-5" />
            </button>
          )}
          <kbd className="hidden sm:inline-flex h-5 select-none items-center gap-1 rounded border bg-white px-1.5 font-mono text-[10px] font-medium text-muted-foreground">
      ...
      <CommandList>
        {isLoading ? (
          ...
          <>
                ...
                {results
                  ...
                  .map((result) => (
                    ...
                    >
                      ...
                      <div className="flex-1 overflow-hidden">
                        <div className="flex items-center gap-2">
                          ...
                          {result.badge && result.badgeColor && (
                            <Badge
                              variant="outline"
                              className={cn(getBadgeStyles(result.badgeColor))}
                            >
                              {result.badge}
                            </Badge>
                          )}
                        ...
                        {result.subtitle && (
                          <div className="truncate text-sm text-muted-foreground group-data-[selected=true]:text-accent-foreground">
                            {result.subtitle}
                          </div>
                        )}
                      ...
                      {result.timestamp && (
                        <span className="text-xs text-muted-foreground whitespace-nowrap group-data-[selected=true]:text-accent-foreground">
                          {result.timestamp}
                        </span>
                      )}
                    </CommandItem>
                      ...
                      <div className="flex-1 overflow-hidden">
                        <div className="flex items-center gap-2">
                          ...
                          {result.badge && (
                            <Badge
                              variant="outline"
                              className={`bg-${result.badgeColor}-100 text-${result.badgeColor}-800 border-${result.badgeColor}-200`}
                            >
                              {result.badge}
                            </Badge>
                          )}
            ...
            {results.some(
              (r) => r.type === "message" || r.type === "conversation",
            ) && (
                ...
                {results
                  ...
                  .map((result) => (
                    ...
                    >
                      ...
                      {result.timestamp && (
                        <span className="text-sm text-muted-foreground group-data-[selected=true]:text-accent-foreground">
                          {result.timestamp}
                        </span>
                      )}
                    </CommandItem>
          ...
          <CommandEmpty>
            <div className="py-6 text-center text-sm text-muted-foreground">
              ...
              <div className="mt-2 text-xs text-muted-foreground">
                ...
                <div className="mb-1">
                  • Find retailers by name, type, status, or location
                </div>
                <div className="mb-1">
                  • Press ⌘K or Ctrl+K to open search anywhere
                </div>
