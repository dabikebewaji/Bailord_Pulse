...
export function useRetailers({
  ...
  // Update retailer rating (the only metric that isn't computed live from
  // orders — see updateRetailerMetrics in the backend)
  const updateMetricsMutation = useMutation({
    mutationFn: ({ id, averageRating }: { id: string; averageRating: number }) =>
      retailerAPI.updateMetrics(id, { averageRating }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['retailers'] });
  ...
  const handleUpdateRating = useCallback(
    async (id: string, averageRating: number) => {
      await updateMetricsMutation.mutateAsync({ id, averageRating });
    },
  ...
  isUpdatingRating: updateMetricsMutation.isPending,
    // Pagination props
    page,
    limit,
    totalPages,
