...
interface UseProjectsOptions {
  // When true, fetches only the projects assigned to the logged-in user's
  // linked retailer business (GET /projects/mine) instead of every project.
  mine?: boolean;
}
...
export function useProjects(options: UseProjectsOptions = {}) {
  ...
  const fetchProjects = useCallback(async () => {
    try {
      setIsLoading(true);
      const response = await api.get<ApiSuccessResponse<ProjectResponse[]>>(mine ? '/projects/mine' : '/projects');
  ...
  const deleteProject = useCallback(async (projectId: string) => {
    try {
      setIsLoading(true);
      // Looked up only to enrich the notification text — this local snapshot
      // can be stale (e.g. right after creating a project), so a miss here
      // must not block the actual delete.
      const project = projects.find(p => p.id === projectId);

      await api.delete(`/projects/${projectId}`);
      setProjects(prev => prev.filter(p => p.id !== projectId));
      toast.success('Project deleted successfully');
      notifications.notifyProjectDeleted(project ?? { id: projectId, name: 'Project' } as Project);
    } catch (error) {
      handleApiError(error as ApiError);
      throw error;
    } finally {
  ...
  const assignRetailers = useCallback(async (projectId: string, retailerIds: string[]) => {
    try {
      ...
      notifications.notifyRetailersAssigned(project ?? { id: projectId, name: 'Project' } as Project, retailerIds.length);
    } catch (error) {
      handleApiError(error as ApiError);
      throw error;
    } finally {
  ...
  const removeRetailer = useCallback(async (projectId: string, retailerId: string, retailerName: string) => {
    try {
      ...
      notifications.notifyRetailerRemoved(project ?? { id: projectId, name: 'Project' } as Project, retailerName);
    } catch (error) {
      handleApiError(error as ApiError);
      throw error;
    } finally {
