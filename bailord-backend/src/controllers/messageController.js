...
// Retailers may only message staff-side users — enforced here, not just
// hidden from the "New Message" list, since the API itself must reject a
// retailer-to-retailer send regardless of what the UI shows.
const canMessage = (senderRole, recipientRole) =>
  senderRole !== 'retailer' || ['admin', 'staff', 'superadmin'].includes(recipientRole);
...
export const sendMessage = async (req, res) => {
  try {
    ...
    if (!canMessage(req.user.role, recipient.role)) {
      return res.status(403).json({ message: 'Retailers can only message staff' });
    }
...
export const getAvailableUsers = async (req, res) => {
  try {
    const currentUserId = req.user.id;
    const q = req.query.q || '';
    const users = await MessageModel.getAvailableUsers(currentUserId, q, req.user.role);
    res.json(users);
  } catch (error) {
...
export const createConversation = async (req, res) => {
  try {
    ...
    if (!canMessage(req.user.role, user.role)) {
      return res.status(403).json({ message: 'Retailers can only message staff' });
    }
