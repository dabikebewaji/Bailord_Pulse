...
// Get all projects (admin/staff/superadmin — gated in the route)
...
// Get only the projects assigned to the logged-in user's linked retailer
// business. Any role can call this; returns an empty list if the caller has
// no linked retailer record (e.g. staff/admin, or a retailer who somehow
// isn't linked).
export const getMyProjects = async (req, res) => {
  try {
    ...
    const projects = await getProjectsForRetailer(retailers[0].id);
    res.status(200).json(projects);
  } catch (error) {
