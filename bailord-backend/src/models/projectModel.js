...
export const assignRetailersToProject = async (projectId, retailerIds) => {
  try {
    ...
    // Bulk insert the assignments. (Previously this also tried to UPDATE a
    // projects.assigned_retailers column that doesn't exist in the schema —
    // every assignment was throwing. The count is now computed on read
    // instead, in getAllProjects/getProjectsForRetailer below, so there's
...
// Fetch all projects, with the assigned-retailer count computed live
// (there's no denormalized column for it — see assignRetailersToProject).
export const getAllProjects = async () => {
  try {
    const [rows] = await pool.query(`
      SELECT p.*, (
        SELECT COUNT(*) FROM project_retailers pr WHERE pr.project_id = p.id
      ) AS assigned_retailers
      FROM projects p
    `);
    return rows;
  } catch (error) {
...
// Fetch only the projects assigned to a given retailer (business record id,
// not a user id) — used for a retailer's own "My Projects" view.
export const getProjectsForRetailer = async (retailerId) => {
  try {
    const [rows] = await pool.query(`
      SELECT p.*, (
        SELECT COUNT(*) FROM project_retailers pr2 WHERE pr2.project_id = p.id
      ) AS assigned_retailers
      FROM projects p
      INNER JOIN project_retailers pr ON pr.project_id = p.id
      WHERE pr.retailer_id = ?
    `, [retailerId]);
    return rows;
  } catch (error) {
    console.error("❌ Database error fetching retailer's projects:", error);
    throw error;
  }
};
