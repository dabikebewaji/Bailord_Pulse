...
export class MessageModel {
  ...
  static async getConversationMessages(user1Id, user2Id) {
    ...
    // Retailers can only reach staff-side users (admin/staff/superadmin) —
    // without this, any retailer could message any other retailer directly.
    // Staff-side roles can message anyone (other staff, or any retailer).
    static async getAvailableUsers(currentUserId, query, currentUserRole) {
      try {
    let sql = `SELECT id, name, company, email, role FROM users WHERE id != ? AND status != 'suspended'`;
        const params = [currentUserId];
        if (currentUserRole === 'retailer') {
          sql += ` AND role IN ('admin', 'staff', 'superadmin')`;
        }
        if (query) {
